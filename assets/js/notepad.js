            ...
            $(".btn-add-layer").click(function () {
                ...
                if (len <= 8) {
                    $('#layers-body').append('<li class="icon-layer icon-selector item' + (len + 1)
                        + '" data-cnt="' + (len + 1) + '"><img src="' + 'assets/images/notepad/layer/layer-' + (len - 2) + '.png"></li>');
                }
            });
            initDraw();
        }
            ...
            try {
                ...
                if (object.item(0).type === 'textbox') {
                    ...
                    if (object.clicked) {
                        ...
                        let textForEditing = new fabric.Textbox(obj.text, {
                            top: object.top + object.height - 5,
            ...
            $(document).ready(function () {
                $('.toogle-menu-tray').on('click', function () {
                    if (!$('#zmmtg-root').hasClass('hidden')) {
                        $('#zmmtg-root').addClass('hidden');
                        $('#zmmtg-root').removeClass('show');
                        $('#nav-tool').css('display', 'none');
                        $('#menu-main').removeClass('hidden');
                        $('.menu-tray').addClass('hidden');
                        $('.sb-right').addClass('hidden');
                    }

                    if ($('.menu-tray').hasClass('hidden')) {
                        $('.menu-tray').removeClass('hidden');
                        $('.sb-right').removeClass('hidden');
                    } else {
                        $('.menu-tray').addClass('hidden');
                ...
                $('.video-mode').click(function () {
                    // const encodedData = btoa('Kiet');
                    // var d1 = document.getElementById('zoom-embed');
                    // d1.insertAdjacentHTML('beforeend', '<iframe src="https://zoom.us/wc/81083122517/join?prefer=0&un=a2lldA==&pwd=306593" sandbox="allow-forms allow-scripts allow-same-origin" allow="microphone; camera; fullscreen"></iframe>');
                    if ($('.menu-tray').hasClass('hidden') == false) {
                        $('.menu-tray').removeClass('hidden');
                        $('.sb-right').removeClass('hidden');
                    }
                    if ($('.meeting-client-inner').hasClass('fullshow')) {
                        $('#zmmtg-root').addClass('hidden');
                    }
...
function addPort(object, canvas, objectID) {
    ...
    else if (object._objects[0].type === "image") { // sơn up lại 6/10
        ports = ['mt']
    }
